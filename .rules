...
3.  **Database**: - Always use **Prisma** for DB operations. - Update `schema.prisma` and run `pnpm db:migrate` for schema changes. - Use **Atlas migrations** for version control and CI/CD. - **Prerequisites**: Ensure PostgreSQL container is running: `docker run --name postgres -e POSTGRES_PASSWORD=postgres -p 5432:5432 -d postgres:16`. - Available migration commands: `pnpm db:migrate` (create), `pnpm db:migrate:named name` (named), `pnpm db:apply` (deploy), `pnpm db:lint` (validate). - Never hardcode SQL queries unless absolutely necessary for performance (and document why).
