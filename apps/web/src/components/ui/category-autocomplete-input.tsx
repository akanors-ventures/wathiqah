 ...
 *
 * Built on a plain popup instead of `<input list>` + `<datalist>` because
 * datalist renders no dropdown at all on iOS Safari and is unreliable on
 * other mobile browsers. Follows the ARIA combobox pattern: focus stays on
 * the input, ArrowUp/ArrowDown move a virtual highlight (aria-activedescendant)
 * over non-tabbable options, and Enter/click commit the highlighted/clicked one.
 */
    ...
    <div ref={containerRef} className="relative">
      <Input
        ...
        onBlur={(e) => {
          ...
          // preventDefault to stop a mouse/touch selection from blurring the
          // input before its click fires, so this never races a real select.
          setOpen(false);
          onBlur?.(e);
        }}
        onKeyDown={(e) => {
        ...
        >
          {filtered.map((suggestion, index) => (
            <button
              ...
              className={
                "block w-full truncate px-3 py-2 text-left text-sm hover:bg-accent hover:text-accent-foreground data-[highlighted=true]:bg-accent data-[highlighted=true]:text-accent-foreground"
              }
